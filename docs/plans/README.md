...
**최종 업데이트**: 2026-04-27 (Phase 12.2 Application — probeapp use cases + ports + fakes 완료)
**현재 단계**: **Phase 12.1+12.2 완료 — `internal/probe/` 도메인 + `internal/application/probe/` (패키지 `probeapp`) use case 레이어. 5 ports (ProbeRepo / ObservationRepo / IncidentRepo / HTTPProber / IDGen) + 4 use cases (RunProbe / EvaluateWindow / QueryProbes / QueryIncidents). EvaluateWindow 가 metric 별 헬퍼 (percentileMS/errorRatePct/consecutiveFailures) 로 분기 + idempotent open/close. testsupport in-memory fakes. 도메인 95.7%, app 86.5% 커버리지. 다음은 12.3 Adapter (`adapters/httpprobe/`) — HTTP 클라이언트 + 에러 분류 구현.**
