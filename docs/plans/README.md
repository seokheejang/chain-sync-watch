...
**최종 업데이트**: 2026-04-27 (Phase 11 슬라이스 1+2 — Helm chart MVP + prod-ready features 완료)
**현재 단계**: **Phase 11 슬라이스 1+2 완료 — Helm chart (`deploy/helm/chain-sync-watch/`) 가 MVP 리소스(Deployment×3 / Service×2 / migrate Job / ConfigMap / Secret / Bitnami pg+redis subchart) 위에 Ingress / HPA / PDB / ServiceMonitor / ExternalSecret 옵셔널 템플릿 + `values.schema.json` + `environments/values.{dev,staging,prod}.yaml` 3종 overlay 추가. autoscaling.enabled=true 시 Deployment.replicas 자동 생략. `helm template` 풀세트 33 리소스 + 환경별(25/31/23) 검증 OK. 다음은 슬라이스 3 (`.github/workflows/release.yml` GHCR push + kubeconform CI) 또는 Phase 12 (probe context).**
