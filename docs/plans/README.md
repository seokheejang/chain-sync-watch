...
**최종 업데이트**: 2026-04-27 (Phase 12.1 Domain — `internal/probe/` 순수 도메인 완료)
**현재 단계**: **Phase 12.1 완료 — `internal/probe/` 신규 bounded context 순수 도메인. `Probe` 애그리게이트 + `Observation` + `Incident` + 6-class `ErrorClass` + 4 종 `ThresholdMetric`. depguard `domain-purity` + `application-boundary` 룰에 추가. 테스트 커버리지 95.7%. 다음은 12.2 Application (`internal/application/probe/`) — RunProbe/EvaluateWindow/Query use cases + ports.**
