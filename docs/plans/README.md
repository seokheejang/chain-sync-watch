...
**최종 업데이트**: 2026-04-27 (Phase 11 종료 — 슬라이스 1~4 모두 완료)
**현재 단계**: **Phase 11 완료 — Helm chart (`deploy/helm/chain-sync-watch/`) MVP + prod-ready 토글(Ingress / HPA / PDB / ServiceMonitor / ExternalSecret) + 환경별 overlay 3종 + JSON Schema 검증 + 릴리스 파이프라인 (`.github/workflows/helm.yml` kubeconform CI + `.github/workflows/release.yml` 4-job DAG GHCR multi-arch image + helm OCI push) + ArgoCD `Application`/`ApplicationSet` (`deploy/argocd/`) + kind 로컬 스모크 (`make kind-smoke`). 다음은 Phase 12 (probe context — API 응답시간/에러 모니터링 별도 bounded context) 또는 follow-up: csw-server `/metrics` 엔드포인트 구현 (ServiceMonitor 가 실제로 스캐핑하려면 필요).**
