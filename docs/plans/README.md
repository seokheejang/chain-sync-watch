...
**최종 업데이트**: 2026-04-27 (Phase 11 슬라이스 1+2+3 — Helm chart + release pipeline 완료)
**현재 단계**: **Phase 11 슬라이스 1+2+3 완료 — Helm chart (`deploy/helm/chain-sync-watch/`) MVP + prod-ready 토글(Ingress / HPA / PDB / ServiceMonitor / ExternalSecret) + 환경별 overlay 3종 + JSON Schema 검증. 릴리스 파이프라인 추가: `.github/workflows/helm.yml` (PR/push → lint + kubeconform per-env, datreeio CRDs-catalog 사용) + `.github/workflows/release.yml` (태그 `v*` push → 4-job DAG: resolve → build-backend + build-web (multi-arch linux/amd64+arm64, GHA cache) → package-helm (Chart.yaml 태그 치환 + `helm push oci://ghcr.io/<owner>/charts` + GitHub Release .tgz 첨부)). 다음은 슬라이스 4 (ArgoCD Application 템플릿, 선택) 또는 Phase 12 (probe context).**
