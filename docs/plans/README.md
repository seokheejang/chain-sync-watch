...
**최종 업데이트**: 2026-04-27 (Phase 12.3 Adapter — adapters/httpprobe/ 완료)
**현재 단계**: **Phase 12.1+12.2+12.3 완료 — `internal/probe/` 도메인 + `internal/application/probe/` use case 레이어 + `adapters/httpprobe/` Prober 구현 (직접 net/http, retry 미사용 — 첫 응답 그대로 측정). 에러 분류: timeout/cancel → ErrorTimeout, *url.Error → ErrorNetwork, 2xx-3xx → ErrorNone, 4xx/5xx → 해당 enum, 1xx → ErrorProtocol. 도메인 95.7%, app 86.5%, 어댑터 91.4% 커버리지. 다음은 12.4 Persistence (`internal/infrastructure/persistence/probe_repo.go` 외 + migration).**
...
| **Phase 9.6** — /schedules CRUD UI: `useCreateSchedule`/`useCancelSchedule` hooks, `/schedules/new` 페이지 + 폼 (chain / metrics / latest_n / cron_expr / timezone), 리스트에 Metrics 컬럼 + Active Badge + Deactivate 확인 Dialog. **cron 형식 fix** — asynq scheduler는 5-field 표준만 받는데 초기 폼이 6-field(`0 */5 * * * *`)로 내보내 scheduler가 `expected exactly 5 fields, found 6` 로 silent fail했음. zod 검증·default·placeholder 모두 5-field로 정정하고 `verification.NewSchedule` 주석에 경고 추가. | (pending commit) |
| **Phase 9.7** — Chain-first UX: `/chains` catalog API 신설(`internal/config/defaults.yaml` 5-chain: Ethereum/Optimism/Base/Arbitrum/Sepolia, `ChainView` DTO, `routes/chains.go`), `useChains` + `useChainSourceCounts`(useQueries 병렬 fetch). `/sources` 페이지를 좌측 chain sidebar(configured 체인 녹색 dot) + 우측 CRUD로 리팩토링, 테이블 Enabled 컬럼 녹색/빨간 dot. `/runs/new`·`/schedules/new` chain input → select dropdown (**configured 체인만 노출**). `SourceFormDialog` chain_id 필드는 sidebar 선택값을 상속(disabled). | (pending commit) |
| **Phase 12.3** — `adapters/httpprobe/` 어댑터: `Prober` 가 `probeapp.HTTPProber` 구현. 직접 `net/http` 사용 (httpx retry 의도적으로 미적용 — probe 는 첫 응답 그대로를 측정해야 신호가 살아 있음). 에러 분류 — `context.DeadlineExceeded`/`Canceled` 둘 다 ErrorTimeout 으로 통합 (shutdown cancellation 도 "응답 못 받음" 신호와 동일), `net.Error.Timeout()` 보강, `*url.Error` (DNS/TCP/TLS/connection refused) → ErrorNetwork, 2xx-3xx → ErrorNone, 4xx/5xx → 해당 enum, 1xx 예외 → ErrorProtocol. ErrMsg multi-byte safe truncation. `New(WithHTTPClient)` Option 패턴, 기본 http.Client.Timeout=30s safety net. 11 테스트 (200/3xx 304/404/503/server-side timeout/connect refused/POST body+headers/malformed URL/long-msg truncation/default GET/pre-cancelled ctx), httptest.Server 활용, 커버리지 91.4%. | (pending commit) |
| **Phase 12.2** — `internal/application/probe/` (패키지 `probeapp`) use case 레이어: 5 ports — `ProbeRepository` / `ObservationRepository` / `IncidentRepository` / `HTTPProber` (Probe 1회 실행 → ProbeResult 반환, error 필드 미사용 — 모든 실패는 ErrorClass) / `IDGen`. 4 use cases — `RunProbe` (FindByID → 비활성 거부 → Prober.Probe → NewObservation 검증 후 Save), `EvaluateWindow` (threshold 별 metric 계산 + open/close, idempotent), `QueryProbes` / `QueryIncidents`. Metric 헬퍼: `percentileMS` (nearest-rank ceil(p×n)), `errorRatePct`, `consecutiveFailures` (트레일링 run-length, len(obs)<N 시 ok=false). `application.Clock` 재사용. `testsupport/` in-memory fakes (FakeProbeRepo/FakeObservationRepo/FakeIncidentRepo/FakeHTTPProber/FakeIDGen). 18 테스트 케이스 (p95 트립/회복/idempotency, error_rate 정확 임계값, consecutive failures broken-streak, multi-threshold 동시 트립, empty window 비전이, evidence 캡), 커버리지 86.5%, race+vet+lint 통과. | (pending commit) |
| **Phase 12.1** — `internal/probe/` 순수 도메인: `Probe` 애그리게이트(ID+Target+Schedule+Thresholds+Enabled), `ProbeTarget`(http/rpc/graphql kind, URL/Method/Headers/Body 검증), `ProbeSchedule`(cron XOR interval, ≥1s), `Threshold`(4 종 metric: latency_p95_ms/latency_p99_ms/error_rate_pct/consecutive_failures + WindowSec + Label), `Observation`(immutable, 6-class `ErrorClass` enum, status↔class consistency 검사, ErrMsgMaxLen=512 truncation), `Incident`(open/close 라이프사이클, Breach, Evidence, defensive copy). 모든 애그리게이트 `Rehydrate` (validation skip — 퍼시스턴스 라운드트립). depguard `domain-purity` + `application-boundary` 에 `internal/probe` 추가. black-box `_test`, 커버리지 95.7%, race+vet+lint 통과. | (pending commit) |
| **Phase 11 슬라이스 4** — ArgoCD GitOps + kind 로컬 스모크: `deploy/argocd/application.yaml` (단일 Application, git source 기본 + OCI 주석 변형, `prune+selfHeal+CreateNamespace+ServerSideApply` syncOptions, finalizer 정리 보장). `deploy/argocd/applicationset.yaml` (list generator 로 dev/staging/prod 3 환경 fan-out). `deploy/argocd/README.md`. `deploy/scripts/kind-smoke.sh` — kind cluster + 로컬 이미지 빌드 + `kind load docker-image` + `helm upgrade --install` (dev overlay + tag=smoke) + `kubectl wait` + port-forward + `/healthz` curl. Make 타겟 `helm-deps` / `helm-lint` / `helm-template` / `kind-smoke` / `kind-smoke-down`. `make helm-lint` 3 환경 통과 + ArgoCD YAML parsing OK. | (pending commit) |
| **Phase 11 슬라이스 3** — 릴리스 파이프라인: `.github/workflows/helm.yml` (PR/push → `deploy/helm/**` 변경 시 `helm dependency update` + `helm lint` + `helm template | kubeconform -strict` per dev/staging/prod 환경 + 풀-feature 패스, datreeio CRDs-catalog 로 ServiceMonitor/ExternalSecret 검증). `.github/workflows/release.yml` 태그 `v*` 트리거 4-job DAG: `resolve` (semver 검증) → `build-backend` + `build-web` (`docker/build-push-action` multi-arch `linux/amd64,linux/arm64`, GHA cache scope 분리) → `package-helm` (Chart.yaml `version`/`appVersion` 을 태그로 sed-치환 후 `helm package` + `helm push oci://ghcr.io/<owner>/charts` + `softprops/action-gh-release` 로 .tgz 첨부). 권한: `contents:write` + `packages:write`, `GITHUB_TOKEN` 만. `workflow_dispatch` 로 재실행 가능. 로컬 검증: kubeconform 3 환경 valid 25/31/17, `helm package` (9.9.9 임시 치환) tgz 생성 OK. | (pending commit) |
| **Phase 11 슬라이스 1+2** — Helm chart MVP + prod-ready features: `deploy/helm/chain-sync-watch/` 스켈레톤(`Chart.yaml`, `_helpers.tpl`, server/worker/web Deployment+Service, migrate Job(helm hook), ConfigMap, Secret, ServiceAccount, NOTES.txt) + Bitnami `postgresql`/`redis` subchart 기본 enabled. 슬라이스 2: `templates/ingress.yaml` (web + `/api` 라우팅, TLS), `templates/hpa.yaml`/`pdb.yaml` (3-component range loop), `templates/servicemonitor.yaml` (Prometheus Operator 스캐폴드 — `/metrics` 미구현 상태에서 미리 와이어업), `templates/externalsecret.yaml` + Secret 템플릿 short-circuit (externalSecrets.enabled=true 시 inline Secret 비렌더 + fail-fast 2종). `values.schema.json` JSON Schema (필수 키·enum 검증), `environments/values.{dev,staging,prod}.yaml` 3종 overlay (dev=single-replica + 번들 subchart, staging=HPA+PDB+Ingress+ServiceMonitor, prod=managed DB + ExternalSecrets-only). Deployment 3종 모두 `spec.replicas` 가 autoscaling.enabled 시 생략 (HPA owns the field). `deploy/README.md` + chart README + NOTES.txt 슬라이스2 토글 안내 보강. `helm template` 풀세트(33 리소스) + 환경별(dev=25, staging=31, prod=23) 검증 OK. | (pending commit) |
| **Phase 10c** — Run 실행 감사 로그 + 보관정책: migration 008 (`runs.anchor_block BIGINT, subjects JSONB, sources_used TEXT[], comparisons_count INT` + `idx_runs_finished_at`), `verification.RunSummary`/`Subject` 값객체 (6 kinds: block/address_latest/address_at_block/erc20_balance/erc20_holdings/snapshot), `Run.RecordSummary`/`Summary` 메서드 (StatusRunning 한정 + defensive copy), `Rehydrate` summary 파라미터. `ExecuteRun` 에 `summaryAccumulator` — 5개 pass(block/addressLatest/addressAtBlock/erc20Holdings/erc20Balance) 각각에서 subject·comparisons 누적 후 Complete 직전 `RecordSummary`. Persistence mapper 양방향 + short-key JSONB(`k/b/a/t/n`). `RunView.Summary`/`SubjectOut` DTO 공개, `web` Run detail에 "Execution summary" 카드(anchor/comparisons/sources/subject 카운트) + "Subjects compared" 섹션(kind별 그룹 log 뷰). Retention: `TaskTypePruneOldRuns` + `PruneOldRunsPayload` + `Handlers.HandlePruneOldRuns` + `RunRepository.PruneFinishedBefore` + `RetentionSchedule` 을 `dbConfigProvider` 가 user schedules 와 함께 방출, `RetentionConfig{RunsDays:14, CronExpr:"0 3 * * *"}` 기본값. **버그 fix**: `gorm:"default:'[]'::jsonb"` 태그가 UPSERT 시 "현재 값이 default 리터럴과 같다"고 판단해 EXCLUDED 컬럼을 skip → completed Run이 pending 단계의 빈 subjects 로 덮어 씌워짐. gorm default 태그 제거 (DB 컬럼의 `DEFAULT` 는 유지). | (pending commit) |
...
| 9.5 | Frontend — /runs 상세 + 생성 폼 (latest_n + manual trigger) | ✅ Done | 9.1-9.4 | [phase-09-frontend.md](./phase-09-frontend.md) |
| 9.6 | Frontend — /schedules CRUD UI + cron 5-field validation fix | ✅ Done | 9.5 | [phase-09-frontend.md](./phase-09-frontend.md) |
| 9.7 | Frontend — Chain-first UX (/chains catalog API, /sources sidebar with configured dot, Run/Schedule chain dropdown configured-only) | ✅ Done | 9.6, 10a | [phase-09-frontend.md](./phase-09-frontend.md) |
| 9.8-9.9 | Frontend pages — address/token plan editors, /sources capability matrix 뷰 | ⬜ Not started | 9.7 | [phase-09-frontend.md](./phase-09-frontend.md) |
| 9.10 | Integration & deploy — docker-compose에 web 서비스 / runtime env | ✅ Done (10b에서 완료) | 10a | [phase-09-frontend.md](./phase-09-frontend.md) |
...
| 10c | Run 실행 감사 로그 + Retention sweep (migration 008, RunSummary/Subject 도메인, ExecuteRun accumulator, RunView.summary + UI, `maintenance:prune_old_runs` 14d cron) | ✅ Done | 6, 7A, 10a | [phase-10-integration-observability.md](./phase-10-integration-observability.md) |
