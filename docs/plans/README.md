...
**최종 업데이트**: 2026-04-24 (Phase 9.6 + 9.7 chain-first UX + 10c run-summary + retention 완료)
**현재 단계**: **Phase 10c 완료 — Run 실행 감사 로그 (`runs.anchor_block / subjects / sources_used / comparisons_count`) + `/chains` catalog API + chain-first `/sources` sidebar + configured-chain filter on Run/Schedule 폼 + `maintenance:prune_old_runs` 14일 보관정책. 성공 run도 "어느 block/address/token을 봤는지" 추적 가능. 다음은 Phase 11 (Helm chart) 또는 Phase 12 (probe context).**
