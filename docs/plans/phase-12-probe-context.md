...
- ✅ **슬라이스 12.3 — Adapter** (2026-04-27): `adapters/httpprobe/` — `Prober` 구조체 (`probeapp.HTTPProber` 구현). 직접 `net/http` 사용 (httpx retry 미사용 — 첫 응답을 그대로 측정해야 신호가 남음). 에러 분류: `context.DeadlineExceeded`/`Canceled` → ErrorTimeout (캐스케이드된 cancellation 도 timeout 으로 통합), `net.Error.Timeout()` → ErrorTimeout, `*url.Error` (DNS/TCP/TLS/connection refused) → ErrorNetwork, 2xx-3xx → ErrorNone, 4xx → ErrorHTTP4xx, 5xx → ErrorHTTP5xx, 1xx (예외) → ErrorProtocol. ErrMsg multi-byte safe truncation (probe.ErrMsgMaxLen). `New(WithHTTPClient)` Option 패턴. 11 테스트 케이스 (200/3xx/404/503/timeout/connect refused/POST body+headers/malformed URL/long-msg truncation/default GET/pre-cancelled ctx), httptest.Server 사용, 커버리지 91.4%.
